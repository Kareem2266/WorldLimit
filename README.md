...
- Celery worker is in Compose but idle — `/generate` and `/export` are fast enough synchronous. Reserved for future 30 s+ jobs (erosion sim, large exports).
- glTF export (OBJ is good enough for now).
- LLM prompt enrichment (`ANTHROPIC_API_KEY` placeholder is in `.env.example`).
- Shared biome logic between JS and Python is currently duplicated; worth a WASM or shared-config pass once it stabilizes.
